...
class controller
{
	...
	public function _processor($options = false)
	{
		...
		if(\lib\dash::is_json_accept() || $force_json || \lib\temp::get('api'))
		{
		...
		if(isset($this->controller()->redirector) && $this->controller()->redirector && !\lib\dash::is_json_accept())
		{
	...
	public function url($_type = null, $_arg = null)
	{
		...
		switch ($_type)
		{
			...
			case 'full':
				$myURL = $myprefix. router::get_domain(). \lib\utility\safe::safe($_SERVER['REQUEST_URI']);
				return rtrim($myURL, '/');
				break;
