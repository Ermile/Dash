...
trait invoice_check_args
{
	public function invoice_check_args($_args, &$args, $_log_meta, $_type = 'insert')
	{
		...
		if(!$title)
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:invoice:title:not:set', $this->user_id, $log_meta);
		...
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:invoice:title:max:length', $this->user_id, $log_meta);
			if($_args['debug']) \dash\notif::error(T_("You must set title less than 50 character"), 'title', 'arguments');
			return false;
		}

		$buyer  = \dash\utility::request('buyer');
		$buyer = \dash\utility\filter::mobile($buyer);
		if(!$buyer)
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:invoice:buyer:not:set', $this->user_id, $log_meta);
			if($_args['debug']) \dash\notif::error(T_("buyer id not found"), 'buyer', 'arguments');
			return false;
		}

		$buyer_detail = \dash\db\users::get(['mobile' => $buyer, 'limit' => 1]);
		...
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:invoice:buyer:not:found', $this->user_id, $log_meta);
			if($_args['debug']) \dash\notif::error(T_("buyer not found"), 'buyer', 'arguments');
			return false;
		}

		// $seller = \dash\utility::request('seller');
		// no now !
		$seller = null;
		...
		// {
			...
			// if($_args['save_log']) \dash\db\logs::set('addons:api:invoice:seller:not:set', $this->user_id, $log_meta);
		...
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:invoice:desc:max:length', $this->user_id, $log_meta);
		...
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:invoice:status:not:set', $this->user_id, $log_meta);
			if($_args['debug']) \dash\notif::error(T_("Invalid status"), 'status', 'arguments');
			return false;
		}


		$date = \dash\utility::request('date');
		if(strtotime($date) === false)
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:invoice:date:not:set', $this->user_id, $log_meta);
			if($_args['debug']) \dash\notif::error(T_("Invalid invoice date"), 'date', 'arguments');
			return false;
		}
		$date = date("Y-m-d", strtotime($date));

		if(\dash\utility::isset_request('temp'))
		{
			if(\dash\utility::request('temp'))
			{
				$temp_invoice = 1;
			}
			else
		...
		$details = \dash\utility::request('details');
		if(!$details)
		{
			if($_args['save_log']) \dash\db\logs::set('addon:api:invoice:detail:not:found', $this->user_id, $log_meta);
		...
		if(!is_array($details))
		{
			if($_args['save_log']) \dash\db\logs::set('addon:api:invoice:details:not:array', $this->user_id, $log_meta);
		...
		if(!$count_detail)
		{
			if($_args['save_log']) \dash\db\logs::set('addon:api:invoice:details:invalid', $this->user_id, $log_meta);
