<?php
namespace content_api\v1\comment\tools;
...
trait comment_check_args
{
	public function comment_check_args($_args, &$args, $_log_meta, $_type = 'insert')
	{
		...
		$post_id = \dash\utility::request('post_id');
		...
		$author  = \dash\utility::request('author');
		if($author && mb_strlen($author) >= 50)
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:comment:author:max:length', $this->user_id, $log_meta);
		...
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:comment:email:max:length', $this->user_id, $log_meta);
		...
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:comment:url:max:length', $this->user_id, $log_meta);
			if($_args['debug']) \dash\notif::error(T_("You must set the url less than 1500 character"), 'url', 'arguments');
			return false;
		}

		$content = \dash\utility::request('content');
		if($content && mb_strlen($content) >= 5000)
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:comment:content:max:length', $this->user_id, $log_meta);
		...
		$meta    = \dash\utility::request('meta');
		...
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:comment:status:max:length', $this->user_id, $log_meta);
		...
		if(!$user_id && \dash\utility::request('user_id'))
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:comment:user_id:max:length', $this->user_id, $log_meta);
			if($_args['debug']) \dash\notif::error(T_("Invalid parameter user_id"), 'user_id', 'arguments');
			return false;
		}

		$type    = \dash\utility::request('type');//'comment','rate'
		if($type && mb_strlen($type) >= 50)
		{
			if($_args['save_log']) \dash\db\logs::set('addons:api:comment:type:max:length', $this->user_id, $log_meta);
	...
	public function comment_make_where($_args, &$where, $_log_meta)
	{
		...
		{
			$where['user_id'] = \dash\coding::decode($user_id);
		}

		if(!$user_id && \dash\utility::isset_request('user_id'))
		{
			$where['user_id'] = null;
		}
	}
